...
import type { PlayModeControlProps } from '../types';
...
/**
 * SecondaryControls 组件
 * 次要控制按钮：随机播放、列表循环、单曲循环
 */
export const SecondaryControls = ({
  playMode,
  onPlayModeChange,
}: PlayModeControlProps) => {
  return (
    <div className="music-player-secondary-controls">
      {/* 随机播放按钮 */}
      <ControlButton
        icon={<RandomIcon />}
        onClick={() => onPlayModeChange('shuffle')}
        focusId="shuffle"
        className={playMode === 'shuffle' ? 'control-btn-active' : ''}
        label="随机播放"
      />
      ...
      <ControlButton
        icon={<CycleIcon />}
        onClick={() => onPlayModeChange('loop-all')}
        focusId="loop-all"
        className={playMode === 'loop-all' ? 'control-btn-active' : ''}
        label="列表循环"
      />
      ...
      <ControlButton
        icon={<LoopOneIcon />}
        onClick={() => onPlayModeChange('loop-one')}
        focusId="loop-one"
        className={playMode === 'loop-one' ? 'control-btn-active' : ''}
        label="单曲循环"
      />
    </div>
  );
